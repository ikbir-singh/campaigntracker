...
const ProjectTables = (props) => {
    ...
    for (let i = 0; i < (tablevalues.length < 20 ? tablevalues.length : 20); i++) {
      ...
      if (element) {
        tableinfo[i] = {
          ...
          views: (element.reel_play ? parseInt(element.reel_play) : 'N/A'),
          likes: (element.reel_like ? parseInt(element.reel_like) : 'N/A'),
          comments: (element.reel_comment ? parseInt(element.reel_comment) : 'N/A'),
        }
      }
  ...
  return (
    <div>
      <Card>
        <CardBody>
          <CardTitle tag="h5">Top Reels</CardTitle>
